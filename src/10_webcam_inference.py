...
#VIDEO_SOURCE = 0
VIDEO_SOURCE = 'source/IMG_3439.MOV'
CONF_THRESHOLD = 0.25
IOU_THRESHOLD = 0.45
INPUT_SIZE = 320
...
if __name__ == "__main__":
    ...
    print(f"[Action] Start the camera or read the video file")

    cap = cv2.VideoCapture(VIDEO_SOURCE)
    if not cap.isOpened():
        print("[Notification] Error: Unable to start Webcam or video file.")
        exit()
    
    print(f"[Notification] Camera start/ Video file read successfully.")
    ...
    while True:
        ...
        # retrieve the size of current frame
        h, w, _ = frame.shape
